...
【实变函数】又名【实分析】, 是标准分析学中的基础课程. 课程内容主要包含 Lebesgue 测度论、积分理论和推广的微积分基本定理. 作为应用, 还将研究可积函数空间的种种性质, 作为泛函分析等更高级的课程的切入点, 例如, \(L^2\) 空间就是天然的无限维 Hilbert 空间, 函数的 Lebesgue 可积性理论还为概率论提供了理论基石, 因此, 实变函数论在现代数学中占据了重要的地位. 通过学习实变函数论, 学生可以掌握分析学的核心工具和方法, 为后续的数学学习和研究打下坚实的基础.
在笔记撰写过程中主要参考了以下教材:
\begin{itemize}
...
\end{itemize}
另外, 已经将项目开源在 Github 中, 地址为 \href{https://github.com/Deferria/26-RealAnalysis}{此处}.
\vspace{\baselineskip}\\
{\Large 鉴于笔者水平有限, 笔记中难免存在错误, 欢迎指正.}
\vspace{\baselineskip}\\
\textcolor{darkpurple}{最后更新于: \today.}
\vspace{5em}\\
我们将从介绍前置知识开始.
\begin{thm}
    [AC]
...
\begin{thm}
    ...
    {连续函数列逼近可测函数}
    \(f\) 是\MeasurableFunc{}, 当且仅当存在连续函数列 \(\{g_n\}_{n=1}^\infty\) 使得 \(g_n\aelim f\).\\
...
\end{rmk}
\begin{prf}
    若存在一列连续函数 \(g_n\), 则 \(g_n\) 都是可测函数. 根据定理\ref{crl:pointwise-limit-measurable}, \(g_n\aelim f\) 可得 \(f\) 可测.\\
    另一边: \(f\) 是可测函数, 则根据 \hyperref[thm:lusin]{Lusin 定理}可取一列闭集 \(F_n\) 和一列连续函数 \(g_n\) 使得 \(f\) 限制在 \(F_n\) 上就是 \(g_n\). 只需令 \(F_n\) 满足 \(\Leb(E\setminus E_n)=\frac{1}{2^n}\), 再利用端点值将 \(g_n\) 延拓到 \(\R\) 上即可得到符合条件的 \(g_n\).
\end{prf}
...
\end{prf}
\begin{rmk}
    可测函数等价于可积函数, 见定理\ref{thm:integrable-iff-measurable}. \\
    但是如果去掉有界性限制, 可测不一定可积. 例如, \(f(x)=\frac{1}{x}\) 在 \((0,1)\) 上可测, 但是不可积. 
    去掉测度有限的限制, 可测函数也不一定可积. 例如, \(f(x)=1\) 在 \(\R\) 上可测, 但是不可积. 这是因为可积的定义是积分不能为 \(\infty\).
\end{rmk}
\begin{ppt}
...
\begin{rmk}
    ...
    \begin{center}
        \(\phi\) \SimpleFunc{} -- \(h\) 有界可测函数 -- \(f\) 非负可测函数 -- 一般的可积函数.
    \end{center}
...
\end{thm}
\begin{prf}
    "($\impliedby$)": 已经证明了可测函数一定可积, 在定理\ref{thm:measurable-of-integrable}.\\
...
\end{thm}
\begin{prf}
    由于零测集上的积分是 0, 所以可以假设 \(f_n\to f\). 设 \(h\) 是一个限制在有限测度集合 \(E'\subseteq E\) 上的有界可测函数, \(0\leq h\leq f\), 定义一列
    \[h_n=\min\{f_n, h\}\]
...
\begin{rmk}
    [Dedicatia]
    收敛可以诱导拓扑. 但是依测度收敛诱导的拓扑非常弱, 它是不可度量化的. 
\end{rmk}
\begin{ins}
    {特征函数的收敛方式}
...
\begin{thm}
    ...
    \hyperref[thm:fatou]{Fatou 引理}, \hyperref[thm:mct]{单调收敛定理}, \hyperref[thm:dct]{控制收敛定理}, \hyperref[thm:lct]{Lebesgue 收敛定理}, \hyperref[thm:vct-1]{Vitali 收敛定理}, \hyperref[thm:vct-2]{一般的 Vitali 收敛定理} 在\hyperref[dfn:convergence-in-measure]{依测度收敛}的条件下仍然成立. 其原因是总可以从依测度收敛的函数列中找到几乎处处收敛的子列, 对子列使用原先的收敛定理, 再根据积分序列作为实数序列的极限唯一性来得到结论.
\end{thm}
\begin{crl}
    {几乎处处收敛与依测度收敛有相同极限}
...
\begin{prf}
    只需令
    \[g_n=n\qty(f\qty(x+\frac{1}{n})-f(x))\]
    这是一列可测函数且 \(g_n\aelim f'\). 根据定理\ref{crl:pointwise-limit-measurable}, \(f'\) 可测
\end{prf}
...
\end{thm}
\begin{prf}
    "(\(\implies\))": 若 \(f\) 是\BVFunc{}, 则
    \[f(x)=(f(x) + \Varia_a^x(f)) - \Varia_a^x(f)\]
    这正是两个单调增函数.\\
    "(\(\impliedby\))": 设 \(f=g-h\), \(g,h\) 是增函数. 取任意一种划分 \(P\):
    \begin{align*}
        \sum_{i=1}^k |f(x_i)-f(x_{i-1})| &= \sum_{i=1}^k |(g(x_i)-g(x_{i-1}))-(h(x_i)-h(x_{i-1}))|\\
        &\leq \sum_{i=1}^k |g(x_i)-g(x_{i-1})| + \sum_{i=1}^k |h(x_i)-h(x_{i-1})|\\
        &= (g(b)-g(a)) + (h(b)-h(a))
    \end{align*}
    所以 \(\Varia_a^b(f) < \infty\), \(f\) 是\BVFunc{}.
\end{prf}
\begin{thm}
    {有界变差函数的连续性}
...
\begin{thm}
    {对绝对值的积分等于全变差等价于绝对连续}
    若 \(f\) 是 \([a,b]\) 上的\BVFunc{}, \(\int_a^b |f'(x)|=V(f)\). 则 \(f\) 是\ACFunc{}.
\end{thm}
\begin{prf}
    由于积分也是一种可测集划分的极限, 所以自然有
    \[\int_u^v|f'|\leq\Varia_u^v(f) \]
    考虑函数
    \[F(x)=\int_a^x |f'|-\Varia_a^x(f)\]
    那么 \(F(a)=F(b)=0\) 但是 \(F(x)\) 又是单调递减的. 所以只可能 \(F(x)=0\) 恒成立. 所以 \(F\) 是连续函数, 满足
    \[|f(u)-f(v)|\leq\Varia_u^v(f)=\int_u^v|f'(t)|\dd{t} \]
    那么当 \(v-u<\delta\) 的时候, 根据测度的绝对连续性, \(\int_u^v|f'|<\epsilon\). 取有限个区间求和仍旧是无穷小量. 从而 \(f\) 是\ACFunc{}.
\end{prf}
\subsection{Lp 空间}
